...
void amikb_gpio_init(void)
{
	...
	HAL_GPIO_WritePin(KBD_GPIO_Port,
		KBD_CLOCK_Pin |
		KBD_DATA_Pin  |
	...
	HAL_GPIO_Init(KBD_GPIO_Port, &GPIO_InitStruct);

	// Lasciamo l'Amiga in RESET finche' non parte la macchina a stati
	HAL_GPIO_WritePin(KBD_GPIO_Port, KBD_RESET_Pin, GPIO_PIN_RESET); // Clear KBD_RESET pin
...
static led_status_t amikb_send(uint8_t keycode, int press)
{
	...
	HAL_GPIO_WritePin(KBD_GPIO_Port, KBD_DATA_Pin, GPIO_PIN_SET); // Normally KBD_DATA pin is HIGH

	// pulse the data line and wait for about 100us
	HAL_GPIO_WritePin(KBD_GPIO_Port, KBD_DATA_Pin, GPIO_PIN_RESET); // KBD_DATA pin is LOW
	udelay(20);
	HAL_GPIO_WritePin(KBD_GPIO_Port, KBD_DATA_Pin, GPIO_PIN_SET); // KBD_DATA pin is HIGH
	udelay(100);
...
void amikb_reset(void)
{
	...
	HAL_GPIO_WritePin(KBD_GPIO_Port, KBD_CLOCK_Pin, GPIO_PIN_RESET); // Clear KBD_CLOCK pin
	mdelay(600);
	HAL_GPIO_WritePin(KBD_GPIO_Port, KBD_RESET_Pin, GPIO_PIN_SET);   // Set KBD_RESET pin
	HAL_GPIO_WritePin(KBD_GPIO_Port, KBD_CLOCK_Pin, GPIO_PIN_SET);   // Set KBD_CLOCK pin
	prev_keycode = 0xff;
	capslk = 0;
	numlk = 0;
...
bool amikb_reset_check(void)
{
	...
	DBG_N("Enter\r\n");
	amikb_direction( DAT_INPUT );
	is_low = HAL_GPIO_ReadPin(KBD_GPIO_Port, KBD_CLOCK_Pin) == GPIO_PIN_RESET ? true : false;
...
static void amim_send_state(void)
{
	//HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET ); // NSS1 low
	
	uint8_t packet[4] = {4, joy0dat_low, joy0dat_high, mouse_flags};

	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET ); // NSS1 low
    HAL_SPI_Transmit(&hspi3, (uint8_t*)&packet, sizeof(packet), 1000);
	while( hspi3.State == HAL_SPI_STATE_BUSY );  // wait xmission complete
	mouse_flags = 0;
	udelay(5);
}

void amim_process(mouse_delta_t *data)
{
    mouse_pos[X] += data->x;
    mouse_pos[Y] += data->y;
    mouse_flags |= 0x80 | (data->buttons&3);
